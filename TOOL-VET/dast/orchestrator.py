...
def main():
    ...
    with tempfile.TemporaryDirectory(prefix="mcp-vetting-") as temp_dir:
        ...
        if is_http:
            ...
            if server_proc.poll() is not None:
                stdout, stderr = server_proc.communicate()
                print("MCP 서버 시작 실패", file=sys.stderr)
                if stdout:
                    print(stdout)
                if stderr:
                    print(stderr, file=sys.stderr)
                print("계속 진행합니다...", file=sys.stderr)
                server_proc = None
        ...
        if openapi_files:
            ...
            if expected_apis:
                print(f"   ✅ {len(expected_apis)}개 예상 API 추출됨")
        ...
        if expected_apis:
            ...
            print(f"✅ {len(tool_api_map)}개 tool에 OpenAPI 원본 경로 패턴 매핑됨")
                    ...
                    for line in f:
                        ...
                        if line:
                            try:
                                ...
                                continue
                print(f"✅ 프록시 로그에서 {len(proxy_entries)}개 API 요청 발견")
            except Exception as e:
        ...
        if proxy_entries and harness_report and harness_report.calls:
            ...
            print(f"✅ {mapped_count}개 tool에 API 매핑됨")
            ...
            for host in hosts:
                ...
                if graphql_endpoint:
                    ...
                    if introspection_enabled:
                        print(f"   ⚠️  Introspection 활성화됨 (보안 취약점 가능)")
                        ...
                        if schema:
                            ...
                            print(f"   ✅ {len(operations)}개 operation 추출됨")
                    else:
                        print(f"   ✅ Introspection 비활성화됨 (안전)")
                    break
        ...
        if expected_apis and proxy_entries:
            ...
            if missing:
                print(f"\n   ⚠️  누락된 API 패턴: {len(missing)}개")
                print(f"   ✅ 매칭된 API 패턴: {len(matched)}개")
                if len(expected_apis) > 0:
                    ...
                    print(f"   📊 수집률: {collection_rate:.1f}%")
